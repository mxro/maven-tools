...
public class MavenRsync {
     ...
     * @throws Exception
     */
    public static void deploy(final DeploymentParameters params) throws Exception {
    
        final Path deploymentDir = Files.createTempDirectory("maven-deployment"); // FileSystems.getDefault().getPath("/data/tmp");
    
        MavenRemoteRepository.downloadRepositoryXml(params.repositoryRemoteUri(), deploymentDir, params.artifact().groupId(), params
                .artifact().artifactId());
        ...
        final String command = "rsync -avz -e \"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" --progress "
                + deploymentDir.toFile().getAbsolutePath() + "/*" + " " + params.rsyncConnectionPath() + deploymentPath;
